...
- **Setup and compilation commands are as follows:**

  ```bash
  cmake -B build # to setup
  cmake --build build -j$(shell nproc || echo 2) # to compile, -j flag to make it parallel across cpu cores
  ```

- **You can also check & use `Makefile.cmake` to see all the commands in cmake:**
  ```bash
  make -f Makefile.cmake build
  ```
...
- **Setup and compilation commands are as follows:**
  ...
  ```bash
  cmake -B build -G Ninja # to setup with ninja backend
  cmake --build # since cmake already knows now your ninja backend, compilation is parallel
  ```

- **You can also check & use `Makefile.cmake` to see all the commands in cmake:**
  ```bash
  make -f Makefile.cmake-ninja build
  ```
